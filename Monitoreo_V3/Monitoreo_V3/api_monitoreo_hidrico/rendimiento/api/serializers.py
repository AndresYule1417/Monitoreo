...
class CreateRendimientoSerializer(serializers.ModelSerializer):
        ...
        if result:                       
            ...
            if(result_serializer):
                ...
                if os.path.exists(fileName): 
                    self.rendimiento(fileName, validated_data)                           
    ...
    def rendimiento(self, fileName, validated_data):
        ...
        df_rh_humedo = pd.read_excel(fileName, sheet_name='RENDIMIENTO HÍDRICO', skiprows=4, usecols="P:AB")    
        df_rh_humedo.rename(columns={'Unnamed: 15':'UA'}, inplace=True)      

        df_rh_seco = pd.read_excel(fileName, sheet_name='RENDIMIENTO HÍDRICO', skiprows=4, usecols="AD:AP")
        ...
        for i in range(df_rh_medio.shape[0]):           
            for j in range(df_rh_medio.shape[1]-1): 
                sql = "insert into rendimiento (id_proyecto, mes, unidad_de_analisis, valor, tipo, fech_digi, hora_digi) \
