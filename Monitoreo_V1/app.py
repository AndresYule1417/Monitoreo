...
def admin_dashboard():
    if 'logueado' not in session or session.get('id_rol') != 1:  # Solo administradores
        return redirect(url_for('login'))
    # Aquí va el código para el dashboard del administrador
...
# Ruta para editar usuario (USUARIO COMÚN)
...
def editar_cuenta():
    ...
    if request.method == 'POST':
        ...
        return redirect(request.referrer or url_for('cuenta'))
...
# NOTICIAS PARA USUARIOS COMUNES 
@app.route('/blog_noticias2')
def blog_noticias2():
   
    return render_template('blog_noticias2.html')
