...
var DialogRef = class {
  constructor(overlayRef, config) {
    ...
    this.config = config;
    this.closed = new import_rxjs2.Subject();
...
var _Dialog = class _Dialog {
  ...
  constructor(_overlay, _injector, _defaultOptions, _parentDialog, _overlayContainer, scrollStrategy) {
    ...
    this._afterAllClosedAtThisLevel = new import_rxjs2.Subject();
    this._afterOpenedAtThisLevel = new import_rxjs2.Subject();
    this._ariaHiddenElements = /* @__PURE__ */ new Map();
    this.afterAllClosed = (0, import_rxjs2.defer)(() => this.openDialogs.length ? this._getAfterAllClosed() : this._getAfterAllClosed().pipe((0, import_operators2.startWith)(void 0)));
...
var MatDialogRef = class {
  constructor(_ref, config, _containerInstance) {
    ...
    this._afterOpened = new import_rxjs3.Subject();
    this._beforeClosed = new import_rxjs3.Subject();
    this._state = MatDialogState.OPEN;
    this.disableClose = config.disableClose;
    this.id = _ref.id;
    _ref.addPanelClass("mat-mdc-dialog-panel");
    _containerInstance._animationStateChanged.pipe((0, import_operators3.filter)((event) => event.state === "opened"), (0, import_operators3.take)(1)).subscribe(() => {
      this._afterOpened.next();
      this._afterOpened.complete();
    });
    _containerInstance._animationStateChanged.pipe((0, import_operators3.filter)((event) => event.state === "closed"), (0, import_operators3.take)(1)).subscribe(() => {
    ...
    (0, import_rxjs3.merge)(this.backdropClick(), this.keydownEvents().pipe((0, import_operators3.filter)((event) => event.keyCode === ESCAPE && !this.disableClose && !hasModifierKey(event)))).subscribe((event) => {
      if (!this.disableClose) {
        event.preventDefault();
  ...
  close(dialogResult) {
    ...
    this._containerInstance._animationStateChanged.pipe((0, import_operators3.filter)((event) => event.state === "closing"), (0, import_operators3.take)(1)).subscribe((event) => {
