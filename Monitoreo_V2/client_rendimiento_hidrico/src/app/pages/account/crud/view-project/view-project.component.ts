  ...
  async getProject_OHTS(id:string){
    ...
    prueba.subscribe({
      next: (result:any) =>{    
        ...
        /*this.map = new google.maps.Map(document.getElementById("map") as HTMLElement, {zoom: 10, center: { lat: 4.2, lng: -75.33 }, mapTypeId: "terrain",});
        let flightPlanCoordinates;
        for(let i=0; i<result.data2.length; i++){
            flightPlanCoordinates = result.data2[i];//7 - 14
